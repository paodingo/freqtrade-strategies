...
def route_proposal(proposal: dict[str, Any], constitution: dict[str, Any]) -> dict[str, Any]:
    ...
    if missing:
        ...
        rules.extend([
            {"rule": "constitution_status_not_agent_approved", "passed": constitution.get("status") == "pending_human_review", "details": {"status": constitution.get("status")}},
            ...
            {"rule": "stage4a_no_auto_execution", "passed": True, "details": {"approval_granted": False}},
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.director_registry:
        ...
        connection.execute(
            ...
            (route["route_id"], route["proposal_id"], route["decision"], json.dumps(route["rule_decisions"], sort_keys=True), 0, route["created_at"]),
        )
        connection.commit()
        connection.close()
