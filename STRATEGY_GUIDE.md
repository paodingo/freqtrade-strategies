...
当前目标不是继续把 V6.5 / V6.6 作为主线对比，而是把历史策略脉络整理清楚，并围绕 V11.30 建立运行诊断、数据新鲜度和真实执行验证链路。

截至当前证据，V11.30 仍是 `insufficient` 状态：

- V11.30 当前 SQLite 中 `trades = 0`。
- V11.30 当前 SQLite 中 `orders = 0`。
- V11.30 15m market data content 当前滞后，zero-trade 不能直接解释为策略失败。
- V10.8.2 snapshot 中存在有限 closed trades / orders，可作为 benchmark data availability 证据。
- 由于 V11.30 没有真实 trades/orders 样本，不能计算 winrate、profit factor、slippage、fee quality、latency quality，也不能做 same-window execution quality comparison。

因此，本说明书只描述策略验证口径和历史版本定位，不得被理解为 V11.30 已通过验证或可以替换 V10.8.2。

## 当前验证口径

后续策略评估应优先回答这些问题：

- V11.30 是否具备实时 15m 和 4h market data coverage。
- V11.30 是否存在策略分析耗时过长导致 missed signals / delayed orders 的风险。
- V11.30 是否实际产生 entry signals，或被 pairlist、protections、filters、资金、交易所/API 状态阻断。
- V11.30 是否能产生真实 dry-run trades/orders 样本。
...
- V11.29：上一阶段真实执行验证对象，已归档为 insufficient / zero-trade 调查对象。
- V11.30：当前观察候选，重点是 market data freshness、decision trace 和 zero-trade cause。
...
这段内容是历史策略解释，不代表当前云端运行面或 V11.30 验证结论。

## V11.30 后续观察指标

V11.30 后续一旦产生真实 dry-run trades/orders，才可以逐步观察：
...
1. 做 V11.30 数据覆盖和运行性能审计。
2. 做 V11.30 market data refresh pipeline diagnosis。
3. 如果确认数据刷新链路修复后仍无交易，再做性能瓶颈审计。
4. 重新观察 V11.30 是否产生真实 trades/orders。
5. 如果 V11.30 仍然没有可解释信号，进入下一策略候选搜索。
