...
这是一个 BTC/USDT:USDT 合约策略实验仓库。当前重点是 V6.3 稳定基线与 V6.5 震荡短线进攻策略的 dry-run 对比，以及新候选 V6.6 选择性箱体边缘策略。
...
| V6.6 | `RegimeAwareV66` | 8082 | dry-run 候选 | 10,000 USDT | 首笔 2,500，只在箱体边缘交易，箱体中部禁开 |
...
- `RegimeAwareV66`：新候选版本；继承 V6.5，但加入 24h/48h 箱体位置过滤，只在上下沿交易，中部不追信号，并在趋势单失效为震荡后更快退出。
