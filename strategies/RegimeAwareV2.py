"""RegimeAware V2 — tighter ranging filter, ATR-based ranging stop, no trend exit signal."""
import logging
...
class RegimeAwareV2(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # V2: require ADX < 20 (confirmed ranging, not weak trend)
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """Exit signals — V2: no 4h trend exit, ATR trailing handles it.
        Only emergency breakdown exit for ranging mode."""

        # Ranging: emergency exit on trend breakdown below EMA200
        dataframe.loc[
            (
                (dataframe["regime_4h"] == RegimeDetector.RANGING)
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        else:  # ranging — ATR-based stop, min 3%, max 6%
            atr_pct = last.get("atr", 0) / current_rate if current_rate > 0 else 0.01
            return max(-0.06, -atr_pct * 2.5)
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        if entry_mode == "ranging":
            ...
            # V2: quick bail if trade is underwater and regime changed to trending
            if current_profit < -0.03 and last.get("regime_4h") == RegimeDetector.TRENDING:
                return "ranging_regime_flipped"
