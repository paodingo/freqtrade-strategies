"""RegimeAware V8 — multi-pair with per-pair hardcoded stops."""
import logging
import traceback
...
class RegimeAwareV8(IStrategy):
    ...
    stoploss = -0.06  # floor
    trailing_stop = False
    use_custom_stoploss = True  # V8: per-pair stop from _PAIR_STOPS

    # Per-pair stop levels based on historical ATR profiles
    _PAIR_STOPS = {
        "BTC/USDT:USDT": -0.04,
        "ETH/USDT:USDT": -0.05,
        "SOL/USDT:USDT": -0.06,
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        return self._PAIR_STOPS.get(pair, self.stoploss)
