      ...
      child = execFileImpl("curl", args, { encoding: "utf8", maxBuffer: 8 * 1024 * 1024 }, (error, stdout, stderr) => {
        ...
        try {
          ...
          resolve({
            ...
            statusText: parsed.status >= 200 && parsed.status < 300 ? "OK" : "Upstream Error",
...
function createEnvAwareFetch({
  ...
  if (typeof directFetch !== "function") throw new Error("A direct fetch implementation is required");
  const nativeProxyEnabled = nodeSupportsNativeEnvProxy(nodeVersion)
    && (execArgv.includes("--use-env-proxy") || env.NODE_USE_ENV_PROXY === "1");
