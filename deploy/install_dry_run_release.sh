...
if [ -s "$CRON_BACKUP" ]; then
  ...
  sed -i '/^TRADE_MONITOR_STATE_FILE=/d; /^TRADE_NOTIFY_DELIVERY_LOG=/d; /^BOT_V1130_SHADOW_DB_FILE=/d' "$cron_next"
  ...
  printf 'BOT_V1130_SHADOW_DB_FILE=%s/user_data/tradesv3_v1130_crash_rebound_shadow.dryrun.sqlite\n' "$LEGACY_ROOT" >> "$cron_with_env"
